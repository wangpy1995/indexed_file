...
static void readSchemas(IndexedFileReader *_this, unsigned short num_schemas, SchemaElement *schemas);

static void readRowGroups(IndexedFileReader *_this, unsigned short num_groups, RowGroup *rowGroups);

static void readKeyValues(IndexedFileReader *_this, int32_t num_kvs, KeyValue *kvs);

static void readImmutableStrings(IndexedFileReader *_this, int32_t num_str, immutable_string *str);

static void readColumnChunks(IndexedFileReader *_this, unsigned short num_columns, ColumnChunk *columns);

static void readColumnMetaDatas(IndexedFileReader *_this, unsigned short num_metas, ColumnMetaData *columnMetaDatas);

static void readStatistics(IndexedFileReader *_this, int32_t num_statistics, Statistics *statistics);

static void readPageEncodingStats(IndexedFileReader *_this, unsigned short num_stats, PageEncodingStats *stats);

static void
readSortingColumns(IndexedFileReader *_this, unsigned short num_sorting_columns, SortingColumn *sortingColumns) ;

void readFileMeta(IndexedFileReader *_this, FileMetaData *metaData) ;
